...
erDiagram
    CONTRIBUTIONS {
        uuid id PK
        uuid sponsor_id FK
        uuid sponsorship_id FK "Links to sponsorship agreement"
    ...
    PAYMENT_CATEGORIES {
        ...
        text name "e.g., Monthly Subsidy, University Tuition, Pocket Money"
...
* **`contributions` (The Physical Inflow):** Manually entered by the Secretary (**UC-10**). This must exactly match the value on the physical check or bank deposit in BDT. Includes an optional free-text `purpose` field for donation description. Optionally linked to a `sponsorship_id` and/or `student_id`.
...
* If a student is in a primary school track (**VLG**), the available selections filter down to `Tuition Subsidy`, `Textbook Grant`, or `Uniform Stipend`. These have `is_repayable = false`, instantly hiding debt mechanics from the data entry form.

### B. Higher Education Loan Program (Repayable Track)

Designed to assist students through university via a revolving fund model.

* **`loans` (The Balance Registry):** Tracks the total outstanding debt, agreement status, and graduation terms.
